...
function nav(){const n=document.getElementById('nav');if(!n)return;n.innerHTML='';try{groups.forEach(g=>{const d=document.createElement('div');d.className='group';d.innerHTML='<div class="group-title">'+(g[state.lang]||g.en)+'</div>';g.items.forEach(([label,key])=>{const a=document.createElement('button');a.type='button';a.className='nav '+(key===state.key?'active':'');a.innerHTML='<span class="nav-icon">'+(icons[key]||'•')+'</span><span>'+T(key)+'</span>';a.onclick=()=>{state.key=key;render()};d.appendChild(a)});n.appendChild(d)})}catch(err){n.innerHTML='<div class="group"><div class="group-title">MENU</div><button type="button" class="nav active" onclick="state.key=\'dashboard\';render()"><span class="nav-icon">▦</span><span>Dashboard</span></button></div>';console.error('Navigation render error',err)}}
...
function render(){setupHeader();const title=document.getElementById('pageTitle');if(title)title.textContent=T(state.key);const subtitle=document.getElementById('pageSubtitle');if(subtitle)subtitle.textContent=T('sub');const brand=document.getElementById('brandSub');if(brand)brand.textContent=T('brand');const lang=document.getElementById('languageSelect');if(lang)lang.value=state.lang;const c=document.getElementById('content');if(!c)return;try{c.innerHTML=state.key==='dashboard'||state.key==='reports'?dashboard():formPage(state.key);if(state.key!=='dashboard'&&state.key!=='reports')renderRecords(state.key,pageTypes[state.key][0])}catch(err){console.error('Page render error',err);c.innerHTML='<div class="panel" style="margin-top:20px"><h2>e-MLA Office</h2><p>Dashboard is loading. Please refresh once.</p></div>'}nav()}
